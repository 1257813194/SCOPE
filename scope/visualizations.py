...
def visualize_full_evolution(
    ...
    for t in range(num_timesteps):
        if t < len(raw_data_2d) and raw_data_2d[t].size > 0:
            ax.scatter(raw_data_2d[t][:, 0], raw_data_2d[t][:, 1], color=colors[t], s=18, alpha=0.35, zorder=1)
    ...
    for u, v, weight, edge_type in edges:
        ...
        if pos_u is not None and pos_v is not None:
            ...
            ax.plot([pos_u[0], pos_v[0]], [pos_u[1], pos_v[1]], **style, linewidth=2.5, alpha=0.9, zorder=2)
    for t in range(num_timesteps):
    ...
    ax.set_xlabel('Dimension 1 (UMAP or Original)', fontsize=14)
    ax.set_ylabel('Dimension 2 (UMAP or Original)', fontsize=14)
    ...
    legend_main.set_title("Cluster Centers", prop={"size": legend_fontsize})
    ...
    legend_edge.set_title("Edge Types",prop={"size": legend_fontsize})
...
def visualize_generated_populations(
    ...
    for t in range(1,len(raw_data)):
        ...
        ax.scatter(xu[:,0] if not adjust_left_right else -xu[:,0], xu[:,1] if not adjust_upside_down else -xu[:,1] + 9, s=1, color = 'gray')
        # if len(raw_data) <=3:
        #     for t_ in range(t+1):    
        ...
        # ax.legend(markerscale=3,loc="lower left")
        xu_ = umap_model.transform(raw_data[t].cpu().numpy()) if umap_model is not None else raw_data[t].cpu().numpy()
        ax.scatter(x=xu_[:, 0] if not adjust_left_right else -xu_[:,0], y=xu_[:, 1] if not adjust_upside_down else -xu[:,1] + 9, s=1, label='real ' + str(observed_time_points[t]), color='gray')
        ax.scatter(xu_generated[:,0] if not adjust_left_right else -xu_generated[:,0], xu_generated[:,1] if not adjust_upside_down else -xu_generated[:,1] + 9, s = 1, color = 'yellow',label='fake ' + str(observed_time_points[t]))
        sns.kdeplot(x=xu_generated[:,0] if not adjust_left_right else -xu_generated[:,0], y=xu_generated[:,1] if not adjust_upside_down else -xu_generated[:,1] + 9, color = 'k')
        if adjust_equal:
            ax.set_aspect('equal', adjustable='box')
        if save_path is not None:
            plt.savefig(
